...
class TutorialScreensThreeVC: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        Defaults.availableVC = "TutorialScreensThreeVC"
    ...
    func setupAnimations() {
        guard let path = Bundle.main.path(forResource: "tutorial3", ofType:"mov") else {
            debugPrint("tutorial3.mp4 not found")
            return
        }
    ...
    @IBAction func nextBtn(_ sender: Any) {
        if selectVC == "MoreVC" {
            guard let vc = UIViewController.viewController(withStoryboard: .TutorialScreens, AndContollerID: "TutorialScreensFourVC") as? TutorialScreensFourVC else {return}
            vc.selectVC = "MoreVC"
            self.navigationController?.pushViewController(vc, animated: true)
        }
        else {
            Router().toSTutorialScreensFourVC()
        }
    }
