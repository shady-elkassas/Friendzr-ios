...
class TutorialScreensOneVC: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        Defaults.availableVC = "TutorialScreensOneVC"
    ...
    @IBAction func nextBtn(_ sender: Any) {        
        if selectVC == "MoreVC" {
            guard let vc = UIViewController.viewController(withStoryboard: .TutorialScreens, AndContollerID: "TutorialScreensTwoVC") as? TutorialScreensTwoVC else {return}
            vc.selectVC = "MoreVC"
            self.navigationController?.pushViewController(vc, animated: true)
        }
        else {
            Router().toSTutorialScreensTwoVC()
        }
    }
