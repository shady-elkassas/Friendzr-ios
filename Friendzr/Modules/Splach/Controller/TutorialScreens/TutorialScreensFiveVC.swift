...
class TutorialScreensFiveVC: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        Defaults.availableVC = "TutorialScreensFiveVC"
    ...
    @IBAction func nextBtn(_ sender: Any) {
        if selectVC == "MoreVC" {
            guard let vc = UIViewController.viewController(withStoryboard: .TutorialScreens, AndContollerID: "TutorialScreensSixVC") as? TutorialScreensSixVC else {return}
            vc.selectVC = "MoreVC"
            self.navigationController?.pushViewController(vc, animated: true)
        }
        else {
            Router().toSTutorialScreensSixVC()
        }
    }
