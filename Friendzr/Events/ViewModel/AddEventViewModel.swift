...
class AddEventViewModel {
    ...
    //MARK:- Add event
    func addNewEvent(withTitle title:String,AndDescription description:String,AndStatus status: String,AndImage image:String,AndCategory categoryId:String,lang:Double,lat:Double,totalnumbert:String,allday:Bool,eventdateFrom:String,eventDateto:String,eventfrom:String,eventto:String, completion: @escaping (_ error: String?, _ data: EventObj?) -> ()) {
        ...
        let bodyData = "Title=\(title)&description=\(description)&status=\(status)&image=\(image)&categorieId=\(categoryId)&lang=\(lang)&lat=\(lat)&totalnumbert=\(totalnumbert)&allday=\(allday)&eventdate=\(eventdateFrom)&eventdateto=\(eventDateto)&eventfrom=\(eventfrom)&eventto=\(eventto)".data(using: .utf8)
